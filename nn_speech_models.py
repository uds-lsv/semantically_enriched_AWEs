...
class WordFeaturizer(object):
    ...
    def __init__(self,
        ...
        self.word_vocab = [w.split() for w in word_vocab]

        # make char vocab


        #all_chars = [w[i] + w[i+1] for w in self.word_vocab for i in range(len(w)-1)]
    ...
    def get_acoustic_form(self,
        ...
        if is_CAE:
            ...
            acoustic_tensor_pad = torch.zeros(spectral_dim, 60 + 1)
            _start_idx = 1

            segment_len = 60 if segment_len > 60 else segment_len
    ...
    def get_symbolic_form(self,
        ...
        # add other tokens
        indices.extend(
            self.char_vocab.lookup_symbol(char) for char in symbol_sequence.split()
        )
...
##### CLASS SemanticWordDataset: A class to handle (batch) data transformation
class SemanticWordDataset(Dataset):
    def __init__(self, dataset_df, featurizer, embedding_lookup):
        """
        Args:
            dataset_df (pandas.df): a pandas dataframe (label, split, file)
            featurizer (SpeechFeaturizer): the speech featurizer
    ...
    def __getitem__(self, index):
        ...
        #try:
        #print(self.embedding_lookup.keys())
        #print(speech_segment.orth.encode('utf-8'))

        #print(speech_segment.orth.encode('utf-8') in self.embedding_lookup.keys())
        try:
            word_emb = self.embedding_lookup[speech_segment.orth.encode('utf-8')]

        except:
            print(speech_segment.orth.encode('utf-8'), speech_segment.word_id)
        ...
        return {
            ...
            'orth_sequence': speech_segment.orth,
            'word_seg_id': speech_segment.word_id,
            'semantic_embedding': word_emb #.encode('utf-8').decode('utf-8')
        }
...
class FrameDropout(nn.Module):
    ...
    def forward(self, x_in):
        ...
        #print(drop_frame_idx, self.dropout_prob)
...
class WordPairEncoder(nn.Module):
    ...
    def __init__(self, acoustic_encoder, encoder_type):
        ...
        hidden_state_dim = acoustic_encoder.hidden_state_dim
        ...
        #     nn.Tanh(),
        #     #nn.Dropout(acoustic_encoder.unit_dropout_prob),
        #     #nn.Linear(in_features=acoustic_embedding_dim, out_features=acoustic_embedding_dim),
        #     #nn.Tanh()
        # )
    ...
    def forward(self, acoustic_ank, acoustic_pos, device=None):
        ...
        if self.encoder_type in ['LSTM', 'BiGRU', 'BiLSTM']:
            ...
            acoustic_emb_ank = self.acoustic_encoder(acoustic_ank, _hidden_0, inference=False)
            acoustic_emb_pos = self.acoustic_encoder(acoustic_pos, _hidden_0, inference=False)
        else:
...
class AcousticEncoderBiGRU(nn.Module):
    def __init__(self,
        ...
        self.frame_dropout_layer = FrameDropout(self.signal_dropout_prob)

        # Convolutional layer  1
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channels=39, #self.spectral_dim,
                out_channels=64, #num_channels[0],
                kernel_size=5, #filter_sizes[0],
                stride=2), #stride_steps[0]),
            nn.BatchNorm1d(64), #num_channels[0]
            nn.ReLU(),
            nn.Dropout(0.2)
        ) #nn.Dropout(self.unit_dropout_prob)
        ...
        # Convolutional layer  1
        # self.conv2 = nn.Sequential(
        #     nn.Conv1d(in_channels=64, #self.spectral_dim,
        #         out_channels=64, #num_channels[0],
        #         kernel_size=5, #filter_sizes[0],
        #         stride=2), #stride_steps[0]),
        #     nn.BatchNorm1d(64), #num_channels[0]
        ...
        self.layer_dropout = nn.Dropout(signal_dropout_prob)
        ...
        self.rnn2 = nn.GRU(
            input_size=2*hidden_state_dim, #spectral_dim,
            hidden_size=hidden_state_dim,
            num_layers=1,
            dropout=unit_dropout_prob,
            bidirectional=True,
    ...
    def forward(self, x_in, initial_hidden_state, inference):
        """The forward pass of the encoder
        ...
        if self.dropout_frames:
            x_in = self.frame_dropout_layer(x_in)

        cnn1_out = self.conv1(x_in)
        ...
        #rec_out, h_n = self.recurrent_layer(rnn_in, h_0)

        rnn1_out, h_n = self.rnn1(rnn_in, h_0)
        rnn2_out, h_n = self.rnn2(self.layer_dropout(rnn1_out), h_0)
        rnn3_out, h_n = self.rnn3(self.layer_dropout(rnn2_out), h_0)
        rnn4_out, h_n = self.rnn4(rnn3_out, h_0)

        rec_vec =  rnn4_out[:, -1, :] #self.dropout(rec_out[:, -1, :]) #
        #lstm_vec = lstm_out[:, -1, :]
        #print(lstm_vec.shape)  >> torch.Size([256, 1024])
        ...
        if inference: # , rnn2_out[:, -1, :], rnn3_out[:, -1, :]
            return rnn1_out[:, -1, :],  acoustic_embedding #rnn2_out[:, -1, :]#, acoustic_embedding
...
class AcousticEncoderGRU(nn.Module):
    def __init__(self,
        spectral_dim=13,
        max_num_frames= 384,
        hidden_state_dim=512,
        ...
        """
        super(AcousticEncoderGRU, self).__init__()
        self.output_dim = output_dim
        self.n_layers = n_layers
        self.hidden_state_dim = hidden_state_dim
        self.unit_dropout_prob=unit_dropout_prob
    ...
    def forward(self, x_in, initial_hidden_state):
        ...
        if self.frame_dropout and self.training:
            x_in = self.frame_dropout_layer(x_in)

        cnn1_out = self.conv1(x_in)

        cnn1_out = cnn1_out.float()

        # permute input for recurrent computation
        rnn_in = cnn1_out.permute(0, 2, 1)
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, acoustic_in, y_sequence_out, inference=False):
        ...
        if self.encoder_type in ['LSTM', 'BiGRU', 'BiLSTM', 'GRU',]:
            ...
            acoustic_emb = self.fc(self.acoustic_encoder(acoustic_in, _hidden_0)) #, inference

        else:
            acoustic_emb = self.fc(self.acoustic_encoder(acoustic_in, inference)) #self.acoustic_encoder(acoustic_in) #
...
class SemanticEncoder(nn.Module):
    def __init__(self, acoustic_encoder, phonetic_decoder, encoder_type='BiGRU', device='cpu'):
        ...
        output_dim = acoustic_encoder.output_dim
        acoustic_embedding_dim = phonetic_decoder.hidden_state_dim
        semantic_embedding_dim = 300

        #self.fc1 = nn.Sequential(
        #    nn.Dropout(acoustic_encoder.unit_dropout_prob),
        #    nn.Linear(in_features=output_dim, out_features=acoustic_embedding_dim),
        #    nn.Tanh(),
            #nn.Dropout(acoustic_encoder.unit_dropout_prob),
            #nn.Linear(in_features=acoustic_embedding_dim, out_features=acoustic_embedding_dim),
            #nn.Tanh()
        ...
        self.form_to_meaning_regressor = nn.Sequential(
            #nn.Dropout(acoustic_encoder.unit_dropout_prob),
            #nn.Linear(in_features=output_dim, out_features=output_dim),
            #nn.ReLU(),
            #nn.Dropout(acoustic_encoder.unit_dropout_prob),
        	nn.Linear(in_features=output_dim, out_features=semantic_embedding_dim),
			#nn.LayerNorm(semantic_embedding_dim),
            nn.Tanh(),
            #nn.ReLU(),
            #nn.Linear(in_features=semantic_embedding_dim, out_features=semantic_embedding_dim),
        )
    ...
    def forward(self, acoustic_in, y_sequence_out, inference=False):
        ...
        if self.encoder_type in ['LSTM', 'BiGRU', 'BiLSTM']:
            ...
            # TODO: fix this! why doing 2 forward passes!!! rnn2, rnn3,
            rnn1, acoustic_emb_1 = self.acoustic_encoder(acoustic_in, _hidden_0, inference=True) #self.fc1(self.acoustic_encoder(acoustic_in, _hidden_0))
            acoustic_emb = acoustic_emb_1 #acoustic_emb_1 #self.fc1(acoustic_emb_1)
            semantic_emb = self.fc2(acoustic_emb_1)

        else:
            #rnn1, rnn2, rnn3, acoustic_emb = self.acoustic_encoder(acoustic_in, inference=True) #self.fc1(self.acoustic_encoder(acoustic_in)) #self.acoustic_encoder(acoustic_in) #
            ...
            acoustic_emb = self.acoustic_encoder(acoustic_in, _hidden_0)
            semantic_emb = self.form_to_meaning_regressor(acoustic_emb)

        if inference:
            return semantic_emb, acoustic_emb #rnn2, rnn3,
        ...
        return outputs, semantic_emb, acoustic_emb
