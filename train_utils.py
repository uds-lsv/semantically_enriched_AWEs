...
def recall(target, ranked_hypotheses, k):
    pred_set = set(ranked_hypotheses[:k])
...
#
def compute_reacall_topk(database_mat, query_mat, database_labels, query_labels, index2word):
    ...
    # query the index
    k = 10 #query_mat.shape[0] # we want to see k nearest neighbors
    ...
    for query_word, candidates in zip(query_labels, ranked_candidates_by_word_label):
        ...
        recall_at_1_values.append(1 if query_word in candidates[:1] else 0)
        recall_at_5_values.append(1 if query_word in candidates[:5] else 0)
        recall_at_10_values.append(1 if query_word in candidates else 0)

        # if query_word in candidates:
        #     recall_at_k_values.append(1)
        #
        # else:
        #     recall_at_k_values.append(0)


    return np.mean(recall_at_1_values),  np.mean(recall_at_5_values),  np.mean(recall_at_10_values)
