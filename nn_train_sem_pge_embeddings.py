...
embedding_lookup = {k.encode('utf-8'): v for k, v in  embedding_lookup.items()}
 ...
 # EXPERIMENT:
 # shuffle semantic representations
#  l = list(embedding_lookup.items())
# random.shuffle(l)
# d = dict(l)
...
if config_args['shuffle_semantic_vectors']:
	new_dict_values = list(embedding_lookup.values())
	random.shuffle(new_dict_values)
	embedding_lookup = dict(zip(embedding_lookup, new_dict_values))#  dict(new_dict_items)


if config_args['binarize_semantic_vectors']:

	#threshold, upper, lower = 0.0, 1.0, 0.0
	# np.where(a>threshold, upper, lower)

	# embedding_lookup = {
	# 	w: np.where(v>threshold, upper, lower).astype(np.float32)
	# 	for w, v in embedding_lookup.items()
	# }

	#bins = [-0.034967266, -0.009959495, 0.0, 0.009959495, 0.034919973]
	bins = [0] #[-0.017034456, 0.016752448]

	# discretize
	embedding_lookup = {
	    w: np.digitize(v, bins, right=False).astype(np.float32) #- 1
	    for w, v in embedding_lookup.items()
	}

	# normalie between -1, 1
	# embedding_lookup = {
	#     w: minmax_scale(v, feature_range=(-1, 1), axis=0, copy=True) #np.digitize(v, bins, right=False).astype(np.float32) - 2.0
	#     for w, v in embedding_lookup.items()
	# }
...
cross_entropy_loss =  nn.CrossEntropyLoss(ignore_index=TRG_PAD_IDX)
...
alpha = config_args['training_hyperparams']['alpha']
beta = config_args['training_hyperparams']['beta']
...
try:
    ...
    for epoch_index in range(num_epochs):
        ...
        for batch_index, batch_dict in enumerate(batch_generator):
            ...
            phon_hat, semantic_embs, acoustic_embs = word_encoder(
            ...
            phon_loss = cross_entropy_loss(phon_hat, phon_tar)
            semantic_loss = MSE_loss(semantic_embs, semantic_tar)/config_args['training_hyperparams']['batch_size']

            loss =  alpha*phon_loss + beta*semantic_loss
            ...
            print(f"{config_args['model_str']}    "
				...
				f"phon-loss: {phon_loss.item():1.4f}   "
				f"sem-loss: {beta*semantic_loss.item():1.4f}   "
        ...
        for batch_index, val_batch_dict in enumerate(batch_generator):
            ...
            phon_tar = val_batch_dict['symbolic_word']
            semantic_tar = val_batch_dict['semantic_embedding']
            ...
            # compute running loss
            val_loss =  alpha*val_phon_loss + beta*val_semantic_loss
            ...
            print(f"{config_args['model_str']}    "
                ...
                f"[{batch_index + 1:>4}/{num_batches:>4}]   "
				f"phon-loss: {val_phon_loss.item():1.4f}   "
                f"sem-loss: {beta*val_semantic_loss.item():1.4f} "
                f"b-loss: {batch_loss:1.4f}   "
        ...
        acoustic_AP_scores.append(mean_acoustic_AP)
...
for i, aAP in enumerate(acoustic_AP_scores):
    #print(aAP, xAP)
    print("Validation performance @ epoch {}: acoustic AP {:.5f}".format(i+1, aAP))


print('Best {} model by A-acoustic AP: {:.5f} epoch {}'.format(
	config_args['model_str'],
	max(acoustic_AP_scores),
    1 + np.argmax(acoustic_AP_scores))
)


print('Best {} model by R@10: {:.5f} epoch {}'.format(
	config_args['model_str'],
	max(recall_at_10_scores),
    1 + np.argmax(recall_at_10_scores))
)
...
best_epoch_model_path = os.path.join(
	...
	config_args['model_str'] +
	'_' +
	str(1 + np.argmax(acoustic_AP_scores)) + '.pth'
)

#best_epoch_model_path = f"{config_args['model_str']}_{1 + np.argmax(recall_at_10_scores)}"

# eval on test set
word_encoder.load_state_dict(torch.load(best_epoch_model_path))
word_encoder.eval()

word_dataset.set_mode('EVA')
...
for batch_index, eval_batch_dict in enumerate(batch_generator):

    # forward pass, get embeddings
    phon_tar = eval_batch_dict['symbolic_word']
    semantic_tar = eval_batch_dict['semantic_embedding']

    # forward pass, get predictions and embeddings
    phon_hat, semantic_embs, acoustic_embs = word_encoder(
        eval_batch_dict['acoustic_word'],
        phon_tar
    )
    ...
    # get word orthographic form for all word in batch
    words_in_batch = eval_batch_dict['orth_sequence']
    ...
    print(f"{config_args['model_str']}    "
        f"EVAL batch [{batch_index + 1:>4}/{num_batches:>4}]   "
    )


print(f"Size of eval set: {len(acoustic_vectors)}")
# at the end of one validation block, compute AP
...
print(f"Eval results acoustic AP: {mean_acoustic_AP:<1.6f}")
